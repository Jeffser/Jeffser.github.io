...
function getDevelopmentStatus(){
    $.getJSON('metadata.json', function(metadata){
        richAlert('<h1 class="tertiaryText title">Development Status - '+metadata['name']+'</h1><div id="development"><div id="development-information"><h1 class="tertiaryText subtitle">'+metadata['development']['status'].charAt(0).toUpperCase()+metadata['development']['status'].slice(1)+'</h1>'+metadata['development']['note']+'</div><img src="../_globalMedia/development/'+metadata['development']['status']+'.gif" alt="'+metadata['development']['status']+'"></div>');
    })
}
//METADATA
$.getJSON('metadata.json', function(metadata){
